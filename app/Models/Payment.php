...
class Payment extends Model
{
	...
	public static function generateCode()
	{
		$prefix = self::PAYMENTCODE . '/' . now()->format('Ymd') . '/' . self::integerToRoman(now()->format('m')) . '/' . self::integerToRoman(now()->format('d')) . '/';
		
		// Generate a unique random string to add to the order code
		$randomString = substr(str_shuffle(str_repeat('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789', 10)), 0, 5);
	
		$orderCode = $prefix . $randomString . '/00001';
		...
		if ($lastOrderCode) {
			$lastOrderNumber = (int)substr($lastOrderCode, -5);
			$nextOrderNumber = sprintf('%05d', $lastOrderNumber + 1);
			$orderCode = $prefix . $randomString . '/' . $nextOrderNumber;
		}
	
		// Check if order code exists in the database, then regenerate if it does
		if (self::_isOrderCodeExists($orderCode)) {
